...
The project itself is a fully fleged LiaScript interpreter that runs on a
...
Whenever you want to trigger a reaload, probably every time the the user saves
...
However there are two additonal features, that can also be used in conjunction
with this branch...
...
The other way around works similar, you will have to overwrite internaly
exposend global function `liaGoto` and move your cursor to the provided line:
...
Unfortunatelly, there is currently no general browser support for text2speech
