...
function storePersitent() {
    let elem = document.getElementsByClassName("persistent");
...
function loadPersistent() {
    ...
    for(let i=0; i<elem.length; i++) {
        for(let j=0; j<bag.childElementCount; j++) {
            if(bag.childNodes[j].id == elem[i].id ) {
                elem[i].replaceWith(bag.childNodes[j]);
                j -= 1;
                break;
            }
        }
