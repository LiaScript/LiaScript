...
function initSettings(app) {
    ...
    app.ports.event2elm.send([SETTINGS, -1, local]);
};
...
class LiaScript {
    ...
    initLogging() {
        ...
        app.ports.event2js.subscribe(function(cmd) {
            ...
            switch (cmd[0]) {
              case "load": {
                  dbLoad(uid, cmd[2], cmd[1], app.ports.event2elm.send);
                  break;
              }
              case "code" : {
              ...
              case "init": {
                  ...
                  document.title = cmd[2];
                  dbInit(uid);
                  dbLoad(uid, "code", cmd[1], app.ports.event2elm.send);
                  dbLoad(uid, "quiz", cmd[1], app.ports.event2elm.send);
                  dbLoad(uid, "survey", cmd[1], app.ports.event2elm.send);
                  break;
              }
              case "settings": {
