...
class LiaDB {
    ...
    store(table, id, data) {
        if(this.channel) {
            this.channel.push("party", {store: table, slide: slide, data: data})
...
function initPreferences(send, data, local=false) {
    console.log("DDDDDDDDDDDDDDDDDDDDDD", data, local);
...
class LiaScript {
    constructor(elem, script, url="", slide=0, channel=null) {
        ...
        this.initChannel(channel, this.app.ports.speech2elm.send);
    ...
    channel_push(data, ok, error) {
        if(this.channel) {
            this.channel.push("party", data)
            .receive("ok", (e) => ok(e))
            .receive("error", (e) => error(e));
    ...
    initEventSystem(jsSubscribe, elmSend) {
        ...
        jsSubscribe(function(cmd) {
            ...
            switch (cmd[0]) {
                case "slide": {
                    self.channel_push( { slide: cmd[1] + 1 } );
                    break;
                }
                case "load": {
