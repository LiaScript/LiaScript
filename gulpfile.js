...
gulp.task('iconfont', (done) => {
    ...
    let iconStream = gulp.src(inputPath + '/**/*.svg').pipe(iconfont({
        fontName: fontName,
        ...
        generateExample = (cb) => {
            ...
            iconStream.on('glyphs', (glyphs, options) => {
                gulp.src(paths.iconTemplatePath + '/icons.css').pipe(consolidate('lodash', {
                    ...
                    className: 'icon',
                    glyphs: glyphs,
                })).pipe(rename('icons.css')).pipe(gulp.dest(targetPath + '/dist'))
                gulp.src(paths.iconTemplatePath + '/preview/icons.html').pipe(consolidate('lodash', {
                    fontName: fontName,
                    className: 'icon',
                    glyphs: glyphs.map(mapGlyphs)
                })).pipe(rename('preview.html')).pipe(gulp.dest(targetPath + '/dist'))
            }).on('finish', cb)
        },
        ...
        generateScssFile = (cb) => {
            ...
            iconStream.on('glyphs', (glyphs, options) => {
                gulp.src(paths.iconTemplatePath + '/icons.scss').pipe(consolidate('lodash', {
                    ...
                    className: 'icon',
                    glyphs: glyphs,
                })).pipe(rename('_icons.scss')).pipe(gulp.dest(targetPath))
            }).on('finish', cb)
        },
        ...
        generateCssFile = (cb) => {
            ...
            iconStream.on('glyphs', (glyphs, options) => {
                gulp.src(paths.iconTemplatePath + '/icons.css').pipe(consolidate('lodash', {
                    ...
                    className: 'icon',
                    glyphs: glyphs,
                })).pipe(rename('icons.css')).pipe(gulp.dest(targetPath))
            }).on('finish', cb)
        },
    ])

    let scssContent = '@import "' + outputPath + "/" + fontName + '/_icons.scss";'
